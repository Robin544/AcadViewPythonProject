...
class Dashboard:
    ...
    def initUI(self, db):
        ...
        self.add_button = Button(self.root, width=20, bg="red", fg="white", text="Add New Note>>",
                               font=self.Font, command=lambda: self.add_callback())
        self.add_button.place(x=25, y=55)
        self.list_all_btn = Button(self.root, width=20, bg="red", fg="white", text="List All Notes",
                                 font=self.Font, command=lambda: self.list_all_callback())
        self.list_all_btn.place(x=295, y=55)
        self.search_label = Label(self.root, text="Search Notes", font=self.Font)
        self.search_label.place(x=20, y=115)
        self.var = StringVar()
        self.search_box = Entry(self.root, width=40, textvariable=self.var, font=self.Font_search_text)
        self.search_box.place(x=20, y=150)
        ...
        self.note_label.place(x=240, y=190)

        self.listbox = Listbox(self.root, selectmode=SINGLE, width=56, font=self.Font_note, height=12)
        self.scroll = Scrollbar(self.root, orient=VERTICAL, command=self.listbox.yview)
        self.listbox['yscroll'] = self.scroll.set

        self.Font_for_label = font.Font(family='Helvetica', size=20, weight='bold')
        self.label = Label(self.root, text="<< Save Your Notes Here >>", font=self.Font_for_label)
        self.label.place(x=115, y=4)

        # self.scroll.pack(side="right", fill="y")
        self.scroll.place(x=615, y=224, height=292)
        self.list_all_callback()

        self.listbox.bind('<<ListboxSelect>>', lambda l: self.edit_callback())
        self.listbox.place(x=5, y=225)
